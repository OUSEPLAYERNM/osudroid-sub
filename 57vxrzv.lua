--[[
    FINAL MONSTER [AIM+ / ORIGINAL PHYSICS]
    - AIM+ dual-mode prediction + dense sweep (accuracy)
    - physics values restored to the EXACT original Flings.lua trio
      (9e7 velocity / 9e8 max rot / 9e9 pin) so the SERVER accepts it
    - ring system at original values
]]
...
local function GetPredictedCFrame(BasePart)
    ...
    if data then
        ...
        if dt > 0.0001 and dt < 0.4 then
            local measured = (BasePart.Position - data.pos) / dt
            if measured.Magnitude > 2500 then
                measured = measured.Unit * 2500
            end
            vel = vel:Lerp(measured, 0.85)
        end
    end

    trackers[BasePart] = { pos = BasePart.Position, t = now }
    ...
    if speed > 60 then
        -- PLANE MODE: long lead
        local lead = math.clamp(pingCache * 2.2 + 0.06, 0.12, 0.4)
            + math.clamp(speed / 220, 0, 0.75)
        lead = math.clamp(lead, 0.12, 1.1)
        offset = vel * lead
        if offset.Magnitude > 260 then
            offset = offset.Unit * 260
        end
    else
        -- PLAYER MODE: short lead, jump axis damped
        local lead = math.clamp(pingCache * 1.2 + 0.02, 0.04, 0.22)
            + math.clamp(speed / 900, 0, 0.1)
        lead = math.clamp(lead, 0.04, 0.3)
        offset = vel * lead
        offset = Vector3.new(offset.X, offset.Y * 0.4, offset.Z)
        if offset.Magnitude > 30 then
            offset = offset.Unit * 30
        end
    end
...
-- ============================================================
-- RING SYSTEM (original Flings.lua values)
-- ============================================================
local infiniteRadius = true
...
ScreenGui.Name = "FinalMonsterAimOrig"
...
Title.Text = "FINAL MONSTER [AIM+ / SERVER]"
...
-- ============================================================
-- FLING LOGIC (AIM+ sweep + ORIGINAL server-proven physics)
-- ============================================================
local function SkidFling(TargetPlayer)
    ...
    -- PHYSICS: exact original Flings.lua trio (balanced = server accepts it)
    local FLING_VELOCITY = 90000000      -- 9e7  (original)
    local FLING_Y_VELOCITY = 900000000   -- 9e7*10 (original launch)
    local FLING_ROT = 900000000          -- 9e8  (max rotation, original)
    local MAX_FORCE = 9000000000         -- 9e9  (original anvil pin)
...
RefreshPlayerList()
UpdateStatus()
Message("Final Monster [AIM+]", "Original server physics restored", 3)