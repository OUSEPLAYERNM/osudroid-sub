...
function Window:SyncDrawer()
	if not self.Mobile then
		self.Nav.Visible = true
		self.Nav.Position = UDim2.fromOffset(0, 0)
		self.DrawerOverlay.Visible = false
		return
	end
	if self.DrawerOpen then
		...
		self.DrawerOverlay.BackgroundTransparency = 0.5
	else
		self.Nav.Visible = false
		self.Nav.Position = UDim2.fromOffset(-self.NavWidth, 0)
		self.DrawerOverlay.Visible = false
	end
end
...
function Library:CreateWindow(opts)
	...
	window.Nav = cn("Frame", { Parent = body, BackgroundTransparency = 1, Size = UDim2.new(0, window.NavWidth, 1, 0), ClipsDescendants = true, ZIndex = 2, Visible = false })
	...
	local function applyLayout()
		local vp = viewport()
		if vp.X < 10 or vp.Y < 10 then return end
		if window.Mobile ~= nil and math.abs(vp.X - window._lastVpX) < 2 and math.abs(vp.Y - window._lastVpY) < 2 then return end
		window._lastVpX, window._lastVpY = vp.X, vp.Y
		window.Mobile = vp.X < 700 or vp.Y < 500 or (UIS.TouchEnabled and not UIS.MouseEnabled)
		if window.Mobile then
			local availW = math.max(vp.X - 12, 260)
			local availH = math.max(vp.Y - 12, 200)
			frame.Size = UDim2.fromOffset(availW, availH)
	...
	task.spawn(function()
		local guard = 0
		while window.Mobile == nil and guard < 25 do
			task.wait(0.2)
...
notify("Naval Warfare", "v2.4.7 loaded clean. RightShift toggles the window.", 5)